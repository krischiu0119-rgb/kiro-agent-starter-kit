...
|-------------|-------------|---------|
| `[PRODUCTION_URL]` | Your live site URL | `https://myapp.vercel.app`, `https://app.example.com` |
...
| `[MANUAL_DEPLOY_COMMAND]` | Fallback manual deploy | `vercel --prod --yes`, `netlify deploy --prod` |
| `[VERIFICATION_PATH]` | Path to check for freshness | `sw.js`, `_next/static/`, `build-info.json` |
| `[APP_ID]` | Platform-specific app identifier | Amplify app ID, Railway project ID |
...
- **Known issues**: Document any platform-specific quirks you've encountered
